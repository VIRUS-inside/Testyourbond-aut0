  ...
  extends AbstractNonStreamingHashFunction
{
  ...
  private static long hashLength0to16(byte[] bytes, int offset, int length) {
    ...
    if (length >= 4) {
      long mul = -7286425919675154353L + length * 2;
      long a = LittleEndianByteArray.load32(bytes, offset) & 0xFFFFFFFF;
      return hashLength16(length + (a << 3), LittleEndianByteArray.load32(bytes, offset + length - 4) & 0xFFFFFFFF, mul);
    }
    if (length > 0) {
      byte a = bytes[offset];
  ...
  private static long hashLength65Plus(byte[] bytes, int offset, int length)
  {
    ...
    long[] v = new long[2];long[] w = new long[2];
    ...
    long mul = -5435081209227447693L + ((z & 0xFF) << 1);
