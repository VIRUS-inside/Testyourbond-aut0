  ...
  implements Constants
{
  ...
  private void writeConstant(int index) {
    ...
    file.println("<H4> <A NAME=cp" + index + ">" + index + "</A> " + Constants.CONSTANT_NAMES[tag] + "</H4>");
    int class_index;
    int name_index;
    String ref;
    switch (tag)
    ...
    case 11: 
      if (tag == 10) {
        ...
        ConstantInterfaceMethodref c1 = (ConstantInterfaceMethodref)constant_pool.getConstant(index, (byte)11);
      ...
      if (method_class.equals(class_name)) {
        ref = "<A HREF=\"" + class_name + "_code.html#method" + getMethodNumber(new StringBuffer().append(method_name).append(signature).toString()) + "\" TARGET=Code>" + html_method_name + "</A>";
      }
      else {
