  ...
  implements Cloneable, PathComponent, ExpressionOwner
{
    ...
    throws CloneNotSupportedException
  {
    ...
    if (wim_lastUsedWalker == this) {
      m_lastUsedWalker = clone;
    }
    if (null != m_nextWalker) {
      m_nextWalker = m_nextWalker.cloneDeep(cloneOwner, cloneList);
    }
    ...
    if (null != cloneList)
    {
      if (null != m_prevWalker) {
        m_prevWalker = m_prevWalker.cloneDeep(cloneOwner, cloneList);
      }
      
    }
    else if (null != m_nextWalker) {
      m_nextWalker.m_prevWalker = clone;
    }
    return clone;
  }
