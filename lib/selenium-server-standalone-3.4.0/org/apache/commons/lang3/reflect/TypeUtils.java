...
public class TypeUtils
{
  ...
  private static boolean isAssignable(Type type, ParameterizedType toParameterizedType, Map<TypeVariable<?>, Type> typeVarAssigns)
  {
    ...
    for (TypeVariable<?> var : toTypeVarAssigns.keySet()) {
      ...
      if ((toTypeArg != null) || (!(fromTypeArg instanceof Class)))
      {
        ...
        if ((fromTypeArg != null) && 
          (!toTypeArg.equals(fromTypeArg)) && ((!(toTypeArg instanceof WildcardType)) || 
  ...
  private static Type unrollVariableAssignments(TypeVariable<?> var, Map<TypeVariable<?>, Type> typeVarAssigns)
  {
    ...
    for (;;)
    {
      ...
      if ((!(result instanceof TypeVariable)) || (result.equals(var))) break;
  ...
  private static boolean isAssignable(Type type, WildcardType toWildcardType, Map<TypeVariable<?>, Type> typeVarAssigns)
  {
    ...
    if ((type instanceof WildcardType)) {
      ...
      lowerBounds = getImplicitLowerBounds(wildcardType);
    ...
    WildcardType wildcardType = toUpperBounds;Type[] upperBounds = wildcardType.length; for (Type[] lowerBounds = 0; lowerBounds < upperBounds; lowerBounds++) { Type toBound = wildcardType[lowerBounds];
    ...
    wildcardType = toLowerBounds;upperBounds = wildcardType.length; for (lowerBounds = 0; lowerBounds < upperBounds; lowerBounds++) { Type toBound = wildcardType[lowerBounds];
  ...
  private static Map<TypeVariable<?>, Type> getTypeArguments(ParameterizedType parameterizedType, Class<?> toClass, Map<TypeVariable<?>, Type> subtypeVarAssigns)
  {
    ...
    for (int i = 0; i < typeParams.length; i++) {
      ...
      typeVarAssigns.put(typeParams[i], typeVarAssigns.containsKey(typeArg) ? 
        (Type)typeVarAssigns.get(typeArg) : typeArg);
    }
    ...
    if (superClass.isInterface())
    {
      ...
      for (Type midType : interfaceTypes) {
        ...
        if ((isAssignable(midClass, superClass)) && 
          (isAssignable(genericInterface, midClass))) {
  ...
  public static boolean isInstance(Object value, Type type)
  {
    ...
    return value == null ? false : (!(type instanceof Class)) || (!((Class)type).isPrimitive()) ? true : 
  ...
  public static Type unrollVariables(Map<TypeVariable<?>, Type> typeArguments, Type type)
  {
    ...
    if (containsTypeVariables(type)) {
      ...
      if ((type instanceof ParameterizedType)) {
        ...
        Map<TypeVariable<?>, Type> parameterizedTypeArguments; if (p.getOwnerType() == null) {
  ...
  public static final ParameterizedType parameterizeWithOwner(Type owner, Class<?> raw, Type... typeArguments)
  {
    Validate.notNull(raw, "raw class is null", new Object[0]);
    Type useOwner;
    Type useOwner; if (raw.getEnclosingClass() == null) {
      Validate.isTrue(owner == null, "no owner allowed for top-level %s", new Object[] { raw });
      useOwner = null; } else { Type useOwner;
      if (owner == null) {
    ...
    Validate.isTrue(raw.getTypeParameters().length == typeArguments.length, "invalid number of type parameters specified: expected %d, got %d", new Object[] {
      Integer.valueOf(raw.getTypeParameters().length), 
      Integer.valueOf(typeArguments.length) });
