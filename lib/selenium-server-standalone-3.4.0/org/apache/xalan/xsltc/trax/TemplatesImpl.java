  ...
  implements Templates, Serializable
{
    ...
    throws TransformerConfigurationException
  {
    ...
    try
    {
      ...
      for (int i = 0; i < classCount; i++) {
        ...
        Class superClass = _class[i].getSuperclass();
      ...
      if (_transletIndex < 0) {
        ErrorMsg err = new ErrorMsg("NO_MAIN_TRANSLET_ERR", _name);
        throw new TransformerConfigurationException(err.toString());
      }
    }
    catch (ClassFormatError e) {
      ErrorMsg err = new ErrorMsg("TRANSLET_CLASS_ERR", _name);
      throw new TransformerConfigurationException(err.toString());
    }
    catch (LinkageError e) {
      ErrorMsg err = new ErrorMsg("TRANSLET_OBJECT_ERR", _name);
      throw new TransformerConfigurationException(err.toString());
    }
  }
  ...
  public synchronized Transformer newTransformer()
    throws TransformerConfigurationException
  {
    TransformerImpl transformer = new TransformerImpl(getTransletInstance(), _outputProperties, _indentNumber, _tfactory);
