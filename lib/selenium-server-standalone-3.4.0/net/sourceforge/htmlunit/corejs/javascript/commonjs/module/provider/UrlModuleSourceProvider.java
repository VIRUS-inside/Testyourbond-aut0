  ...
  extends ModuleSourceProviderBase
{
  ...
  private ModuleSource loadFromPathList(String moduleId, Object validator, Iterable<URI> paths) throws IOException, URISyntaxException
  {
    ...
    for (URI path : paths) {
      ModuleSource moduleSource = loadFromUri(path
        .resolve(moduleId), path, validator);
    ...
    throws IOException
  {
    ...
    URLValidator applicableValidator; if ((validator instanceof URLValidator)) {
    ...
    try {
      ...
      if ((applicableValidator != null) && 
        (applicableValidator.updateValidator(urlConnection, request_time, urlConnectionExpiryCalculator)))
      {
        close(urlConnection);
        return NOT_MODIFIED;
      }
      ...
      return new ModuleSource(getReader(urlConnection), 
        getSecurityDomain(urlConnection), uri, base, new URLValidator(uri, urlConnection, request_time, urlConnectionExpiryCalculator));
    }
  ...
  {
    return urlConnectionSecurityDomainProvider == null ? null : urlConnectionSecurityDomainProvider
    ...
    implements Serializable
  {
    ...
    boolean updateValidator(URLConnection urlConnection, long request_time, UrlConnectionExpiryCalculator urlConnectionExpiryCalculator)
      throws IOException
    {
    ...
    private boolean isResourceChanged(URLConnection urlConnection) throws IOException
    {
      if ((urlConnection instanceof HttpURLConnection)) {
        return 
          ((HttpURLConnection)urlConnection).getResponseCode() == 304;
      }
    ...
    private long calculateExpiry(URLConnection urlConnection, long request_time, UrlConnectionExpiryCalculator urlConnectionExpiryCalculator)
    {
      ...
      if (cacheControl != null) {
        ...
        if (-1 != max_age) {
          ...
          long apparent_age = Math.max(0L, response_time - urlConnection
            .getDate());
          long corrected_received_age = Math.max(apparent_age, urlConnection
      ...
      return urlConnectionExpiryCalculator == null ? 0L : urlConnectionExpiryCalculator
