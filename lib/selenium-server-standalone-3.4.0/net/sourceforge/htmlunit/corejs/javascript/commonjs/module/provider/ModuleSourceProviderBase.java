  ...
  implements ModuleSourceProvider, Serializable
{
    ...
    throws IOException
  {
    ...
    for (int i = 0; i < ilength; i++) {
      String path = ensureTrailingSlash(
        (String)ScriptableObject.getTypedProperty(paths, i, String.class));
      try {
        URI uri = new URI(path);
        if (!uri.isAbsolute()) {
          uri = new File(path).toURI().resolve("");
        }
        ModuleSource moduleSource = loadFromUri(uri
          .resolve(moduleId), uri, validator);
        if (moduleSource != null) {
          return moduleSource;
        }
  ...
  protected abstract ModuleSource loadFromUri(URI paramURI1, URI paramURI2, Object paramObject)
    throws IOException, URISyntaxException;
