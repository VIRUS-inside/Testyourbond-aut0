...
public class Main
{
    ...
    implements ContextAction, QuitAction
  {
    ...
    public Object run(Context cx) {
      ...
      if (type == 1) {
        Main.processFiles(cx, args);
      } else if (type == 2) {
        Main.evalInlineScript(cx, scriptText);
      } else {
        throw Kit.codeBug();
      }
      return null;
  ...
  static void processFiles(Context cx, String[] args)
  {
    ...
    for (String file : fileList) {
      try {
        ...
        Context.reportError(ToolErrorReporter.getMessage("msg.couldnt.read.source", file, ioex
          .getMessage()));
        exitCode = 4;
  ...
  static void evalInlineScript(Context cx, String scriptText) {
    try {
      ...
      ex.printStackTrace();
      String msg = ToolErrorReporter.getMessage("msg.uncaughtJSException", ex
        .toString());
      Context.reportError(msg);
      exitCode = 3;
    }
  ...
  public static String[] processOptions(String[] args)
  {
    for (int i = 0;; i++) {
      ...
      if (arg.equals("-version")) {
        i++; if (i == args.length) {
          String usageError = arg;
          break;
        }
        try
        {
          version = Integer.parseInt(args[i]);
        } catch (NumberFormatException ex) { int version;
          String usageError = args[i];
          break; }
        int version;
        ...
        } catch (NumberFormatException ex) { int opt;
          String usageError = args[i];
          break; }
        int opt;
        if (opt == -2)
    ...
    global.getOut().println(ToolErrorReporter.getMessage("msg.shell.usage", Main.class
      .getName()));
    exitCode = 1;
    return null;
  }
  ...
  private static void initJavaPolicySecuritySupport() {
    Throwable exObj;
    try {
      Class<?> cl = Class.forName("net.sourceforge.htmlunit.corejs.javascript.tools.shell.JavaPolicySecurity");
      ...
      exObj = ex;
    } catch (IllegalAccessException ex) { Throwable exObj;
      exObj = ex;
    } catch (InstantiationException ex) { Throwable exObj;
      exObj = ex;
    } catch (LinkageError ex) { Throwable exObj;
      exObj = ex;
    }
    ...
    throws IOException
  {
    if ((filename == null) || (filename.equals("-"))) {
      ...
      Charset cs; Charset cs; if (charEnc != null) {
      ...
      while (!hitEOF) {
        ...
        for (;;)
        {
          ...
          } catch (IOException ioe) { String newline;
  ...
  public static void processFileNoThrow(Context cx, Scriptable scope, String filename)
  {
    try {
      ...
      Context.reportError(ToolErrorReporter.getMessage("msg.couldnt.read.source", filename, ioex
        .getMessage()));
      exitCode = 4;
    } catch (RhinoException rex) {
      ToolErrorReporter.reportException(cx.getErrorReporter(), rex);
  ...
  private static byte[] getDigest(Object source) {
    ...
    if (source != null) { byte[] bytes;
      if ((source instanceof String)) {
        byte[] bytes;
        try { bytes = ((String)source).getBytes("UTF-8");
        } catch (UnsupportedEncodingException ue) { byte[] bytes;
  ...
  private static Object readFileOrUrl(String path, boolean convertToString)
    throws IOException
  {
    return SourceReader.readFileOrUrl(path, convertToString, shellContextFactory
  ...
  static class ScriptReference extends SoftReference<Script> {
    ...
    ScriptReference(String path, byte[] digest, Script script, ReferenceQueue<Script> queue) { super(queue);
      this.path = path;
      this.digest = digest;
    }
  }
  
  static class ScriptCache extends LinkedHashMap<String, Main.ScriptReference> {
    ...
    Main.ScriptReference get(String path, byte[] digest)
    {
      ...
      Main.ScriptReference ref = (Main.ScriptReference)get(path);
      if ((ref != null) && (!Arrays.equals(digest, digest))) {
        remove(path);
        ref = null;
      }
      return ref;
