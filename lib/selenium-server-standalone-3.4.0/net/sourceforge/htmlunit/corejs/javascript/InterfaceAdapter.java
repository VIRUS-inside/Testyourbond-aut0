...
public class InterfaceAdapter
{
  ...
  static Object create(Context cx, Class<?> cl, ScriptableObject object)
  {
    ...
    if (adapter == null) {
      ...
      if ((object instanceof Callable))
      {
        ...
        if (length == 0) {
          throw Context.reportRuntimeError1("msg.no.empty.interface.conversion", cl
            .getName());
        }
        if (length > 1) {
          ...
          for (int i = 1; i < length; i++) {
            if (!methodName.equals(methods[i].getName())) {
              throw Context.reportRuntimeError1("msg.no.function.interface.conversion", cl
    ...
    return VMBridge.instance.newInterfaceProxy(proxyHelper, cf, adapter, object, topScope);
  }
  
  private InterfaceAdapter(ContextFactory cf, Class<?> cl)
  {
    proxyHelper = VMBridge.instance.getInterfaceProxyHelper(cf, new Class[] { cl });
  }
  ...
  Object invokeImpl(Context cx, Object target, Scriptable topScope, Object thisObject, Method method, Object[] args) {
    ...
    if (args == null) {
      ...
      int i = 0; for (int N = args.length; i != N; i++) {
        Object arg = args[i];
