  ...
  extends ClassEmitterTransformer
{
  ...
  public CodeEmitter begin_method(int access, Signature sig, .Type[] exceptions) {
    new CodeEmitter(super.begin_method(access, sig, exceptions)) {
      public void visitFieldInsn(int opcode, String owner, String name, String desc) {
        .Type towner = TypeUtils.fromInternalName(owner);
        switch (opcode) {
        case 180: 
          if (filter.acceptRead(towner, name)) {
            helper(towner, InterceptFieldTransformer.readMethodSig(name, desc)); return;
          }
          
          break;
        case 181: 
          if (filter.acceptWrite(towner, name)) {
            helper(towner, InterceptFieldTransformer.writeMethodSig(name, desc)); return;
          }
          break;
        }
